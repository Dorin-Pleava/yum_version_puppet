...
class Error < StandardError; end
  class Version < Numeric
    ...
    def self.parse(ver)
      match, epoch, upstream_version, yum_revision = *ver.match(REGEX_FULL_RX)

      raise ValidationFailure, "Unable to parse '#{ver}' as a yum version identifier" unless match

      new(epoch.to_i, upstream_version, yum_revision).freeze
    end
    ...
    def self.compare_yum_versions(mine, yours)
    ...
    def initialize(epoch, upstream_version, yum_revision)
      @epoch            = epoch
      @upstream_version = upstream_version
      @yum_revision  = yum_revision
    end

    attr_reader :epoch, :upstream_version, :yum_revision

    def to_s
      s = @upstream_version
      s = "#{@epoch}:" + s if @epoch != 0
      s = s + "-#{@yum_revision}" if @yum_revision
      s
    end
    alias inspect to_s
    ...
    def eql?(other)
      other.is_a?(Version) &&
        ...
        @yum_revision.eql?(other.yum_revision)
    end
    alias == eql?
    ...
    def <=>(other)
      ...
      if cmp == 0
        ...
        if cmp == 0
          cmp = compare_yum_revision(other)
        end
      end
      cmp
    end
    ...
    def compare_upstream_version(other)
      ...
      Version.compare_yum_versions(mine, yours)
    end

    def compare_yum_revision(other)
      mine = @yum_revision
      yours = other.yum_revision
      Version.compare_yum_versions(mine, yours)
    end

    # Version string matching regexes
    REGEX_EPOCH   = '(?:([0-9]+):)?'
    # alphanumerics and the characters . + - ~ , starts with a digit, ~ only of yum_revision is present
